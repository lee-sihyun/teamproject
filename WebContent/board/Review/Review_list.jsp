...
<%
	...
	//Review 테이블에 저장된 전체 게시글의 갯수를 반환하는 DAO 클래스의 메소드 호출
	int totalReview = ReviewDAO.getDAO().getReviewTotal(search, keyword);//전체 게시글의 갯수를 저장하기 위한 변수
	...
	int totalBoard=ReviewDAO.getDAO().getReviewTotal(search,keyword);
	
	
	//시작 행번호와 종료 행번호를 전달하여 Review 테이블에서 페이지에 
	//응답될 게시글 목록을 검색하여 반환하는 DAO 클래스의 메소드 호출
	List<ReviewDTO> ReviewList=ReviewDAO.getDAO().getReviewList(startRow, endRow, search, keyword);
...
.box-b .col-a .input.Review-search .txt-field {
	height: 40px;
	line-height: 38px;
	font-size: 16px;
}

.input.Review-search .txt-field {
	margin-right: 7px
}
...
.input.Review-search .txt-field .text {
	height: 30px;
	line-height: 30px;
}
...
.cs-page .box-b .col-a .btn.Review-search {
	height: 40px;
	line-height: 38px;
}

.cs-page .btn.Review-search {
	width: 101px;
}
...
<body>
	...
	<div id="side1">
		...
		<div id="main">
			...
			<form name="frmlist" id="frmlist" action="<%=request.getContextPath()%>/index.jsp?workgroup=board/Review&work=Review_list" method="get">
			...
			<div class="main">
				<div class="m2">
					...
					<div class=table1 id="ReviewList" class="ReviewList">
						<table>
							<colgroup>
								<col class="length">
								<col>
							...
							<tbody>
								<tr class="toggle-Review answer">
									<td align="center" class="title">
										<% if(Review.getStatus()==9) {//일반글인 경우 %> 
										삭제된 글입니다.
										<%} else {%>
											<a href="<%=request.getContextPath()%>/index.jsp?workgroup=board/Review&work=Review_detail&num=<%=Review.getNum()%>&pageNum=<%=pageNum%>"><%=Review.getName() %></a>
										<%} %>
									</td>
									<td align="center" class="data002"><%=Review.getId()%></td> 
									<td align="center" class="data003"><%=Review.getRating()%></td> 
									<td align="center" class="data004">
										<% if(currentDate.equals(Review.getRegDate().substring(0,10))) { %> <%=Review.getRegDate().substring(11, 19) %> <% } else { %> <%=Review.getRegDate().substring(0, 19) %> <% } %>
									</td>
								</tr>
								<% } %>
								<% } %>
					...
					<div class="paging">
						...
						<a href="<%=request.getContextPath()%>/index.jsp?workgroup=board/Review&work=Review_list&pageNum=1&search=<%=search%>&keyword=<%=keyword%>">[처음]</a> <a href="<%=request.getContextPath()%>/index.jsp?workgroup=board/Review&work=Review_list&pageNum=<%=startPage-blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[이전]</a>
						<% } else { %>
						[처음][이전]
						<% } %>
						...
						<% if(pageNum!=i) { %>
						<a href="<%=request.getContextPath()%>/index.jsp?workgroup=board/Review&work=Review_list&pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>">[<%=i %>]
						</a>
						<% } else { %>
						...
						<% if(endPage!=totalPage) { %>
						<a href="<%=request.getContextPath()%>/index.jsp?workgroup=board/Review&work=Review_list&pageNum=<%=startPage+blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[다음]</a> <a href="<%=request.getContextPath()%>/index.jsp?workgroup=board/Review&work=Review_list&pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">[마지막]</a>
						<% } else { %>
						[다음][마지막]
						<% } %>
					...
					<br> <br>
						...
						<a href="<%=request.getContextPath()%>/index.jsp?workgroup=board/Review&work=Review_write"style="pointer-events:auto ; cursor:auto;">리뷰쓰기 </a> 
						<!-- <button type="button" id="write" value="리뷰쓰기">리뷰 쓰기</button> -->
...
<script type="text/javascript"> 
	if($("#write").click(function() {
    	location.href="<%=request.getContextPath()%>/index.jsp?workgroup=board/Review&work=Review_write";
    }));
</script>