...
<%
	...
	*/
	...
	int totalOrder = pageDAO.getDAO().getAllPageTotal(search, keyword);
	...
	int productSum= (Integer)session.getAttribute("productSum"); 
...
<div id="content">
	...
	<form action="" method="post" name="eProductSearchForm"
		...
		<!-- QA_list1종료 -->
		<div class="section" id="QA_list2"  style="
    height: 700px;">
			...
			<div class="mBoardtypeListgScrollgCell" style="
    height: 570px;
">
				...
				<table class="eChkColor">
					...
					<tr>
						...
						<td><%=order.getId()%></td>
					<!--  -->	<td><%= 
					DecimalFormat.getCurrencyInstance().format(order.getProductSum())%></td>
		...
		<div>
				...
				%>
				<a
					href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=order&pageNum=<%=startPage + blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[다음]</a>
				<a
					href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=order&pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">[마지막]</a>
				<%
					} else {
				%>
				[다음][마지막]
