...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	int totalDelivery3 = pageDAO.getDAO().getPageTotal3(search, keyword);
	int totalPage = (int) Math.ceil((double) totalDelivery3 / pageSize);
	...
	if (endRow > totalDelivery3) {
		endRow = totalDelivery3;
	}

	List<pageDTO> pageList3 = pageDAO.getDAO().getPageDeliver3List(startRow, endRow, search, keyword);

	int number = totalDelivery3 - (pageNum - 1) * pageSize;
...
<div id="page" style="text-align: center;padding-top: 50px;">
					...
					%>
					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=delivery3&pageNum=1&search=<%=search%>&keyword=<%=keyword%>">[처음]</a>
					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=delivery3&pageNum=<%=startPage - blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[이전]</a>
					<%
						} else {
					%>
					[처음][이전]
					...
					%>
					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=delivery3&pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>">[<%=i%>]
					</a>
					<%
						} else {
					%>
					...
					%>
					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=delivery3&pageNum=<%=startPage + blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[다음]</a>
					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=delivery3&pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">[마지막]</a>
					<%
						} else {
					%>
					[다음][마지막]
