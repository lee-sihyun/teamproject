...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	int totalDelivery2 = pageDAO.getDAO().getPageTotal2(search, keyword);
	int totalPage = (int) Math.ceil((double) totalDelivery2 / pageSize);
	...
	if (endRow > totalDelivery2) {
		endRow = totalDelivery2;
	}

	List<pageDTO> pageList2 = pageDAO.getDAO().getPageDeliver2List(startRow, endRow, search, keyword);

	int number = totalDelivery2 - (pageNum - 1) * pageSize;
...
<link href="css/admin_delivery2.css" rel="stylesheet" type="text/css">
<style  type="text/css"> 
#content {
	width: 1280px;
	height: 1029px;
	...
	<form action="" method="post"  name="deliveryform" id="deliveryform">
		...
		<div class="section" id="QA_deposit2"
			...
			<div class="mTab typeTab eTab"></div>
					...
					<button  type="submit"id="delivery" class="btnCtrl" >배송완료</button>	
				...
				<div class="mBoardtypeOrdergScrollgCellSingle"
					...
					<td><%=order.getId() %></td>
					 <!--  <DecimalFormat.getCurrencyInstance().format(order.getProductSum()) %-->
...
<div id="page" style="text-align: center;padding-top: 50px;">
					...
					%>
					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=delivery2&pageNum=1&search=<%=search%>&keyword=<%=keyword%>">[처음]</a>
					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=delivery2&pageNum=<%=startPage - blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[이전]</a>
					<%
						} else {
					%>
					[처음][이전]
					...
					%>
					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=delivery2&pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>">[<%=i%>]
					</a>
					<%
						} else {
					%>
					...
					%>
					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=delivery2&pageNum=<%=startPage + blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[다음]</a>
					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=delivery2&pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">[마지막]</a>
					<%
						} else {
					%>
					[다음][마지막]
...
$("#delivery").click(function () {
	...
	$("#deliveryform").attr("action","<%=request.getContextPath()%>/admin/order/order_delivery_action2.jsp");
	$("#deliveryform").submit();
