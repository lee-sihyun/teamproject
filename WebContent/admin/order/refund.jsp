...
<%
	...
	int totalRefund = pageDAO.getDAO().getRefundTotal(search, keyword);
	int totalPage = (int) Math.ceil((double) totalRefund / pageSize);
	...
	if (endRow > totalRefund) {
		endRow = totalRefund;
	}

	List<pageDTO> pageList4 = pageDAO.getDAO().getRefundList(startRow, endRow, search, keyword);

	int number = totalRefund - (pageNum - 1) * pageSize;
...
<form  method="post"  name="refundform" id="refundform">
			...
			<ol class="tab" style="margin-bottom: 0px; margin-top: 0px;">
				<li id="ship_tab_no_pay_cnt"><a
					...
					id="no_pay_cnt" style="width: 206px;"><span
						class="thumb deposit"><strong>환불</strong><span
		...
		<div class="section" id="QA_deposit2"
			...
			<div id="tabNumber" class="tabCont" style="display: block;">
					...
					<div class="gLeft">
						...
						<button type="submit" id="refund" class="btnCtrl">환불처리</button>
				...
				<div class="mBoardtypeOrdergScrollgCellSingle"
					...
					<table id="searchResultList" border="1" class="eChkTr"
						...
						<tr>
							...
							<th scope="col" style="widht: 50px;">아이디</th>
							<th scope="col">금액</th>
...
<div id="page" style="text-align: center;padding-top: 50px;">
					...
					%>
					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=refund&pageNum=1&search=<%=search%>&keyword=<%=keyword%>">[처음]</a>
					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=refund&pageNum=<%=startPage - blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[이전]</a>
					<%
						} else {
					%>
					[처음][이전]
					...
					%>
					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=refund&pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>">[<%=i%>]
					</a>
					<%
						} else {
					%>
					...
					%>
					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=refund&pageNum=<%=startPage+blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[다음]</a>
					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=order&work=refund&pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">[마지막]</a>
					<%
						} else {
					%>
					[다음][마지막]
...
$("#refund").click(function () {
	...
	$("#refundform").attr("action","<%=request.getContextPath()%>/admin/order/refund_action.jsp");
	$("#refundform").submit();
