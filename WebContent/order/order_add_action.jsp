...
<%
	...
	//각 상품당 주문한 수량만큼 재고에서 차감하기
	String[] volumeList=request.getParameterValues("volInput");
	String[] itemNoList=request.getParameterValues("itemNo");
	int length=volumeList.length;
	
	for(int i=0;i<length;i++) {
		ProductDAO.getDAO().minusStock(Integer.parseInt(volumeList[i]), Integer.parseInt(itemNoList[i]));
	}
	
	//주문 상세 등록 처리페이지로 이동
	response.sendRedirect(request.getContextPath()+"/order/order_detail_add_action.jsp?orderNo="+orderNo);
%>