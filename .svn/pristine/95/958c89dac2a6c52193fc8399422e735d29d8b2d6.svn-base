...
public class Utility {
    ...
    //새로운 비밀번호를 생성하여 반환하는 메소드 - 임시비밀번호 생성이기 때문에 비밀번호 재생성 할 수 있게
    public static String randomPassword() {
        return UUID.randomUUID().toString().replace("-", "").substring(0, 8).toUpperCase();
    }
    
    //문자열을 전달받아 문자열에 존재하는 태그 관련 문자열을 모두 제거하고 반환하는 메소드
    public static String stripTag(String source) {
        //Pattern : 정규표현식을 저장하여 사용하기 위한 클래스
        //Pattern.compile(String regEx) : 정규표현식을 저장하고 있는 Pattern 인스턴스를 생성하여 반화하는 메소드
        ...
        //htmlScript.matcher(String source) : 정규표현식과 전달 문자열을 비교처리하기 위한 matcher 인스턴스를 반환하는 메소드
