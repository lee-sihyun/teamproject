...
<br>
	...
	<% if(loginUser1 != null && loginUser1.getId().equals(QnA.getId())) { %>
		<button type="button" id="removeBtn">글삭제</button>&nbsp;
		<button type="button" id="modifyBtn">글변경</button>&nbsp;
	<% } %>
	<% if(loginUser1 !=null && loginUser1.getId().equals("admin")) {//로그인 상태인 경우 %>	
...
$("#modifyBtn").click(function() {
	$("#QnAForm").attr("action", "<%=request.getContextPath()%>/index.jsp?workgroup=board/QnA&work=QnA_modify&num=<%=num%>&pageNum=<%=pageNum%>");
	$("#QnAForm").submit();
});

$("#replyBtn").click(function() {
	$("#QnAForm").attr("action", "<%=request.getContextPath()%>/index.jsp?workgroup=board/QnA&work=QnA_write&ref=<%=QnA.getRef()%>&refStep=<%=QnA.getRefStep()%>&refLevel=<%=QnA.getRefLevel()%>");
	$("#QnAForm").submit();
});
