...
<%
	...
	int totalFAQ = FAQDAO.getDAO().getFAQTotal(search, keyword);
...
<div id="content">
	...
	<div class="section" id="QA_list2">
		...
		<!-- 표영역 -->
		<form
			action="<%=request.getContextPath()%>/admin/index.jsp?workgroup=board&work=faq_list"
			...
			<div class="mButtongCenter">
				...
				<div class="writeBtn" style="text-align: right;">
					<button type="button" id="writeBtn"
						style="margin-bottom: 10px; height: 31.979166px;">글쓰기</button>
				</div>
				...
				<div class="mBoardtypeListgScrollgCell">
					...
					<table class="eChkColor">
						...
						<tr>
							...
							/admin/index.jsp?workgroup=board&work=faq_detail&num=

							<%=faq.getNum()%>&pageNum=<%=pageNum%>&search=<%=search%>&keyword=

							<%=keyword%>"><%=faq.getTitle()%></a>
...
<%=board.getNum()%>&pageNum=<%=pageNum%>&search=<%=search%>&keyword=<%=keyword%>"><%=board.getSubject() %></a>		--%>
							...
							<td>
								<%
									if (currentDate.equals(faq.getRegdate().substring(0, 10))) {
								%> <%=faq.getRegdate().substring(11, 19)%> <%
 	} else {
 %> <%=faq.getRegdate().substring(0, 19)%> <%
 	}
 %>
...
/admin/index.jsp?workgroup=board&work=faq_list&pageNum=1

&search=<%=search%>&keyword=<%=keyword%>">[처음]</a>

					<a
						href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=board&work=faq_list&pageNum
...
=<%=startPage - blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[이전]</a>
	...
	/admin/index.jsp?workgroup=board&work=faq_list&pageNum=
	...
	<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">[마지막]</a>
				...
				<div id="btn">
					...
					<select name="search" style="height: 22px;">
...
$("#writeBtn").click(function() {
	location.href="<%=request.getContextPath()%>/admin/index.jsp?workgroup=board&work=faq_write";
});
