...
public class QnADAO extends JdbcDAO {
	...
	// => 검색대상이 없는 경우 QnA 테이블에 저장된 전체 게시글의 갯수를 검색하여 반환하는 메소드
	public int getQnATotal(String id) {
		...
		try {
			con = getConnection();
			String sql = "select count(NUM) from QNA where ID = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
			System.out.println(rs.next());
			System.out.println("전체개수; : "+rs.getInt(1));
					
			/*
			 * if(rs.next()){ System.out.println("여기 왜 안 들어오니"); rows = rs.getInt(1);
			 * System.out.println("dsfdsfsdf:"+rows); }
			 */
			rows = rs.getInt(1);
			System.out.println(rows);
		} catch (SQLException e) {
			System.out.println("[에러]getQnATotal() 메소드의 SQL 오류 = " + e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
		return rows;
	...
	// => 유저 로그인 했을 때 그 사람만의 글을 보여주는 메소드
	public List<QnADTO> getQnAIDList(String id) {
		//System.out.println("11111111111");
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		List<QnADTO> QnAID = new ArrayList<QnADTO>();
		try {
			con = getConnection();
			...
			System.out.println("[에러]getQnAList() 메소드의 SQL 오류 = " + e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
		return QnAID;
	...
	// 시작 행번호와 종료 행번호를 전달받아 QnA 테이블에 저장된 게시글 목록을 페이지 단위로 검색하여 반환하는 메소드
	// => 테이블에 저장된 행에 대한 페이징 처리
	public List<QnADTO> getQnAList(int startRow, int endRow, String id) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			con = getConnection();
				...
				String sql = "select * from (select rownum rn, temp.* from (select * from QnA  where id = ? order by ref desc,ref_step) temp) where rn between ? and ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, id);
			...
			System.out.println("list 길이 : "+QnAList.size());
		} catch (SQLException e) {
			System.out.println("[에러]getQnAList() 메소드의 SQL 오류 = " + e.getMessage());
		} finally {
			close(con, pstmt, rs);
	...
	public int addQnA(QnADTO QnA) {
		//System.out.println("다오에서 확인 : "+QnA.getTitle());
		//System.out.println("다오에서 확인 : "+QnA.getContent());
		//System.out.println("다오에서 확인 : "+QnA.getCategory());
		//System.out.println();
		String inquiry = "";
		if(QnA.getRefStep() == 0) {//새글일때
			inquiry = "NO";
		}else {
			inquiry = "YES";
		}
...
/*
		// refLevel를 전달받아 QnA 테이블에 저장된 Inquiry 상태를 변경해서 저장하는 메소드
		public int getInquiry(int ref, int refStep) {
			Connection con = null;
			PreparedStatement pstmt = null;
		int rows = 0;
			try {
				...
				String sql= "update QnA set inquiry="+"Yes"+" where ref=? and ref_step>?";
				pstmt = con.prepareStatement(sql);
				...
				System.out.println("[에러]getrefLevel() 메소드의 SQL 오류 = " + e.getMessage());
			} finally {
				close(con, pstmt);
			}
			return rows;
	...
	*/	
	...
	public int modifyQnA(QnADTO QnA) {
		...
		try {
			...
			String sql = "UPDATE QNA SET TITLE=?, CONTENT=?, CATEGORY=? WHERE NUM=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, QnA.getTitle());
			pstmt.setString(2, QnA.getContent());
			pstmt.setString(3, QnA.getCategory());
	...
	//게시글 현재 시간을 전달받아 QnA 테이블에 저장된 게시글을 검색하여 반환하는 메소드
	public QnADTO getDate(int num) {
		//System.out.println("날짜확인 : " + num);
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		QnADTO QnA = null;
