...
<form name="insertForm" id="orderForm" style="width: 1180px; margin: 0 auto;">
						...
						<td class="cart_name">
							<div>
								...
								<div style="display: inline-block; height: 33px; padding: 40px;">
									<a id="itemName" href=""><%=item.getName()%></a>
								</div>
							</div>
						</td>
						<td class="cart_volume">
							<input type="hidden" value="<%=cart.getCartNo()%>" name="checkCartNo">
							<input type="hidden" value="<%=item.getItemNo() %>" name="itemNo">							
							<input type="text" class="volInput" name="volInput" value="<%=cart.getVolume() %>" readonly="readonly">
						</td>
						<td class="cart_price">
		...
		<div class="totalPrice" style="padding: 30px; font-size: 18px; text-align: center; margin-bottom: 70px;">
			...
			총 <%=CartDAO.getCartDAO().getCartNumber(loginUser.getId()) %>개의 주문 금액 &nbsp;&nbsp;=&nbsp;&nbsp;&nbsp;
			<input type="text" name="productSum" style="font-weight: bold; font-size: 26px; width: 90px; text-align: center;" class="totalPriceInput" readonly="readonly" value="<%=productSum%>">
			&nbsp;&nbsp;원
		</div>
		<h1>주문 고객 정보</h1>
		<div class="orderDiv">
			<table>
				...
				<tr>
					...
					value="<%=loginUser.getPhone().substring(0, 2)%>-<%=loginUser.getPhone().substring(2,6)%>-<%=loginUser.getPhone().substring(6)%>"> 
					</td>
				</tr>
				<tr>
					<th>이메일</th>
			...
			<table>
				...
				<tr>
					...
					<td>
						...
						<label for="information" class="checkboxInfo">위 내용을 회원정보에 반영합니다. (주소/휴대폰번호)</label>
					</td>
				</tr>
			</table>
		</div>
		...
		<div class="orderDiv">
			<table id="payment">
				<tr>
					<th>합계 금액</th>
					<td><%=DecimalFormat.getCurrencyInstance().format(productSum) %></td>
				</tr>
				<tr>
					<th>배송비</th>
		...
		<div class="orderDiv" style="padding: 0; margin-bottom: 0; border-top: 1px solid #000; border-bottom: 0.5px solid #e7e7e7;">
			<table>
				<tr>
					...
					<td>
						<input type="radio" name="wayOfPayment" id="creditCard" class="radioPayment" value="creditCard">
						<label for="creditCard" class="radioPayment">신용카드</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<input type="radio" name="wayOfPayment" id="accountTransfer" class="radioPayment" value="accountTransfer">
						<label for="accountTransfer" class="radioPayment">계좌이체</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<input type="radio" name="wayOfPayment" id="virtualAccount" class="radioPayment" value="virtualAccount">
						<label for="virtualAccount" class="radioPayment">가상계좌</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<input type="radio" name="wayOfPayment" id="phonePayment" class="radioPayment" value="phonePayment">
		...
		<div class="totalPrice" style="padding: 35px 20px;">
			<span style="font-weight: bold;">최종 결제 금액  <%=DecimalFormat.getCurrencyInstance().format(productSum) %></span>
		</div>
		<div id="payAgreement">
	...
	if($("#basicAddr").is(":checked")) {
		...
		insertForm.deliverPhone.value="<%=loginUser.getPhone()%>";
	}
	...
	$("#goPay").click(function() {
		...
		$("#orderForm").attr("action","<%=request.getContextPath()%>/order/order_add_action.jsp");
		$("#orderForm").submit();
	});
   	</script>