...
">게시글</h2>
	...
	<table>
		...
		<tr>
			<th>작성자</th>
			<td>	<%=notice.getId() %> <%-- 로그인 사용자가 관리자인 경우 --%> <%-- <% if(loginMember!=null && loginMember.getStatus()==9) { %>
				[<%=board.getIp() %>]
				<% } %>--%></td>
		</tr>
		<tr>
...
$("#modifyBtn").click(function() {
	$("#boardForm").attr("action", "<%=request.getContextPath()%>/admin/index.jsp?workgroup=board&work=board_modify");
	$("#boardForm").submit();
});


$("#listBtn").click(function() {
	$("#boardForm").attr("action", "<%=request.getContextPath()%>/admin/index.jsp?workgroup=board&work=board_list");
	$("#boardForm").submit();
});
</script>
