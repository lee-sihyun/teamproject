	...
	public List<testDTO> getTestList(int startRow, int endRow, String search, String keyword) {
		...
		try {
			...
			if (keyword.equals("")) {
				...
				String sql = "select * from (select rownum rn, temp.* from (select * from lushuser where " + search+ " like '%'||?||'%'  order by num desc) temp) where rn between ? and ?";
	...
	//회원정보 변경 
	 public int updateStatus(){
	        ...
	        try {
	            ...
	            String sql=" UPDATE lushuser set status = CASE when (sysdate-logdate)>=180 then 2 when (sysdate-logdate)<180 then 1 end WHERE id != 'master'";
	            ...
	            System.out.println("[에러] updateStatus SQL 에러 "+e.getMessage());
	        } finally {
	            close(con, pstmt);
	        }
	 ...
	 public int selectAll(String id) {
	        ...
	        try {
	            ...
	            String sql="select * from lushuser";
